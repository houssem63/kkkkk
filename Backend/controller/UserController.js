...
module.exports = {
  ajouter: async (req, res) => {
    try {
      ...
      if (!req.files["Image"]) {
        this.personnelimage =
          "https://secure.gravatar.com/avatar/03723a218a9152e9bad38a84058e21d7?s=192&d=mm&r=g%202x";
      } else {
      ...
      if (!req.files["CopierPermis"]) {
        ...
        this.copierpermis =
          url + "/images/" + req.files["CopierPermis"][0].filename;
      }
      const user = req.body;
      ...
      if (!count) {
        ...
        await User_Role.bulkCreate([{ userID: 1, roleID: 1 }]);
        return res.status(200).json({
          ok: true,
          msg: "inscription effuctuée avec succès",
        });
      }
      if (req.body.roles) {
        global.user;
        ...
        if (req.body.Matfiscale !== undefined) {
          const findMatFiscale = await User.findAll({
            where: { Matfiscale: req.body.Matfiscale },
          });

          if (findMatFiscale[0]) {
            return res.json({
              msg: "Matricule fiscale déja utilisé",
              ok: false,
            });
          }
        }
        ...
        role.forEach(async (element) => {
          await User_Role.bulkCreate([{ userID: user.ID, roleID: element.ID }]);
        });
        ...
        if (req.body.Abonnement !== undefined) {
          ...
          if (lastabonnement.length !== 0) {
            return res.json({
              msg: "La société à déjà un abonnement pour la periode spécifiée",
              ok: false,
            });
          }
        ...
        return res
          .status(200)
          .json({ ok: true, msg: "utilisateur ajouté  avec succès" });
      }
      ...
      User.create(this.user)
        .then(async (resq) => {
          ...
          if (user_role) {
            res
              .status(200)
              .json({ ok: true, msg: "inscription effuctuée avec succès" });
          }
          /* */
        })
        .catch((err) => {
    ...
    try {
      ...
      if (responce === 0) {
        return res
          .status(200)
          .json({
            msg: "Vous ne pouvez pas supprimer cet utilisateur !",
            ok: false,
          });
      }
      ...
      res
        .status(200)
        .json({ msg: "Utilisateur supprimé avec succès", ok: true });
    } catch (e) {
  ...
  Update: async (req, res) => {
    try {
      ...
      const userresponce = await User.findOne({
      ...
      if (userresponce) {
        userresponce.update({
          ...
          DateDeNaissance: req.body.DateDeNaissance
            ...
            : userresponce.DateDeNaissance,
          NumCNSS: req.body.NumCNSS ? req.body.NumCNSS : userresponce.NumCNSS,
          SituationFamilialle: req.body.SituationFamilialle
            ? req.body.SituationFamilialle
            : userresponce.SituationFamilialle,

          CopierPermis: this.copierpermis
            ? this.copierpermis
            : userresponce.CopierPermis,

          NomPC: req.body.NomPC ? req.body.NomPC : userresponce.NomPC,
          PrenomPC: req.body.PrenomPC
            ? req.body.PrenomPC
            : userresponce.PrenomPC,
          TelPersonnelContact: req.body.TelPersonnelContact
            ? req.body.TelPersonnelContact
            : userresponce.TelPersonnelContact,
          FaxPersonnelContact: req.body.FaxPersonnelContact
            ? req.body.FaxPersonnelContact
            : userresponce.FaxPersonnelContact,
          AdresseEmailPersonnel: req.body.AdresseEmailPersonnel
            ? req.body.AdresseEmailPersonnel
            : userresponce.AdresseEmailPersonnel,
          Regfiscale: req.body.Regfiscale
            ? req.body.Regfiscale
            : userresponce.Regfiscale,
          Function: req.body.Function
            ? req.body.Function
            : userresponce.Function,
          SocieteID: req.body.SocieteID
            ? req.body.SocieteID
            : userresponce.SocieteID,
        });
      }
  ...
  Getbyid: (req, res) => {
    ...
    if (!hasrole("Administrateur", token) && !(hasrole("Société",token))) {
  ...
  auth: async (req, res, next) => {
    try {
      ...
      global.adminindex;
      this.adminindex = this.user[0].roles.findIndex(
        (r) => r.Libelle === "Administrateur"
      );
      if (!(this.adminindex !== -1 && this.user[0].Status !== false)) {
        ...
        if (this.societe === null) {
          if (this.societe[0].Status === false) {
            return res.json({
              msg: "Le compte  de votre société n'est pas actif",
              ok: false,
            });
          }
        }
      ...
      const token = jwt.sign(
        {
          ...
          societeID: this.user[0].societeID
            ? this.user[0].societeID
            : this.user[0].ID,
          roles: this.user[0].roles,
        },
        SECRET,
        { expiresIn: "4h" }
      );
      ...
      const resdate = await User.update(
        { Dernierconnection: dernierconnection },
        {
          where: {
            ID: this.user[0].ID,
          },
      ...
      res
        ...
        .json({
          ...
          expiresIn: "14400",
          ok: true,
          msg: "Connexion effectué avec succès",
        });
    } catch (err) {
    ...
    try {
      ...
      if (record == null) {
        return res.status(401).json({
          message:
            "Jeton introuvable. Veuillez réessayer le processus de réinitialisation du mot de passe.",
        });
      }
      ...
      return res.status(200).json({
        message:
          "Mot de passe réinitialiser. Veuillez vous connecter avec votre nouveau mot de passe.",
      });
    } catch (error) {
      console.log(error);
    }
  ...
  validateCaptcha: async (req, res) => {
    try {
      ...
      // const secretkey = RECAPTCHA_SECRET_KEY; //the secret key from your google admin console;

      //token validation url is URL: https://www.google.com/recaptcha/api/siteverify
      // METHOD used is: POST

      const url = `https://www.google.com/recaptcha/api/siteverify?secret=${RECAPTCHA_SECRET_KEY}&response=${token}&remoteip=${req.connection.remoteAddress}`;
      ...
      if (token === null || token === undefined) {
        res
          .status(201)
          .send({ success: false, message: "Token vide ou invalide" });
    ...
    try {
      ...
      const users = await User.findAll({
        include: [
          ...
          {
            model: HistoriqueEmbauches,
            as: "historiqueEmbauches",
          },
        ],
        order: [["ID", "ASC"]],
        limit: pageSize,
  ...
  recherchecompte: async (req, res) => {
    try {
      global.user;
      this.user = await User.findOne({ where: { Email: req.body.email } });
      ...
      if (this.user) {
        ...
        const output = `
                Une demande de réinitialisation de votre mot de passe pour votre compte a été faite sur ${APP_NAME}.
                Vous pouvez maintenant vous connecter en cliquant sur le lien ci-dessous ou en le copiant dans votre navigateur :
                http://${DOMAIN}/changement-mot-de-passe?token=${encodeURIComponent(
          token
        )}&mail=${this.user.Email}
        
                Ce lien ne peut être utilisé qu'une seule fois pour vous connecter et vous mènera à la page pour changer votre mot de passe. Il expirera dans une heure et rien ne se passera s'il n'est pas utilisé.
        ...
        let transporter = nodemailer.createTransport({
          ...
          auth: {
            name: NAME,
            user: MAIL_HOST_USER, // generated ethereal user
            pass: MAIL_HOST_PASS, // generated ethereal password
          },
          tls: {
            rejectUnauthorized: false,
          },
        ...
        // setup email data with unicode symbols
        let mailOptions = {
          from: MAIL_HOST_USER, // sender address
          to: this.user.Email, // list of receivers
          subject: FORGOT_PASS_SUBJECT_LINE, // Subject line
          //  text: 'Hello world?', // plain text body
          html: output, // html body
        };
        ...
        transporter.sendMail(mailOptions, (err, info) => {
          ...
          res
            ...
            .json({
              msg:
                "Les instructions de réinitialisation de mot de passe ont été envoyer à votre mail",
              ok: true,
            });
        });
      } else {
  ...
  GetonePersonnelbyid: async (req, res) => {
    try {
      ...
      if (personnel) {
        res.status(200).json({ personnel: personnel[0] });
      }
    } catch (err) {
      res.status(500).json({ err: "Erreur au niveau de serveur : " + err });
    }
  ...
  resetPassword: async (req, res) => {
    ...
    if (record == null) {
      return res.status(401).json({
        message:
          "Le jeton a expiré. Veuillez réessayer la réinitialisation du mot de passe",
      });
    }
    var userFound = await User.findOne({ where: { Email: mail } });
    return res.status(200).json({
      message: "Afficher le formulaire de la réinitialisation du mot de passe",
      showForm: true,
      ResetToken: record,
