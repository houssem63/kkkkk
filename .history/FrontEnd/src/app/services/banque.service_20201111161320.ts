...
this.getallbanquesub.next([...this.banques]);
  ...
  edit(banque:Banque){
    this.http.put<{ok:boolean ,msg:string}>(BACKEND_URL+`update/${banque.ID}`,banque).subscribe((res=>{
if(res.ok ===true){
  const updatebanque = [...this.banques];
  const oldbanque = updatebanque.findIndex(b => b.ID === banque.ID);
  updatebanque[oldbanque] = banque;
  this.banques = updatebanque;
  this.getallbanquesub.next([...this.banques]);
}
    }))
  }
